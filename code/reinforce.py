...
def return_summary_index(terminated_way, probs_numpy, probs_torch, \
                        sample_method="greedy", if_test= False, max_num_of_sents=4,min_num_of_sents=1, eps = 1e-3):
    """
    ...
    if max_num_of_sents <= 0:
        ...
        if sample_method == "sample" and (not if_test):
            ...
            loss = sum(loss_list)/(float(len(loss_list)) + 1e-8)
        elif sample_method == "sample" and if_test:
            probs_torch = probs_torch.squeeze()
            assert len(probs_torch.size()) == 1
...
def is_termination(prob_masked):
...
class ReinforceReward():
    def __init__(self, terminated_way, std_rouge=False, rouge_metric="all", b=5, rl_baseline_method="greedy", loss_method=1):
        """
        :param std_rouge:
        :param rouge_metric:
        :param b:
        ...
        self.terminated_way = terminated_way

    def train(self, probs, doc, min_num_of_sents=1, max_num_of_sents=3, max_num_of_bytes=-1, prt=False):
        """
        :return: training_loss_of_the current example
        """ 
        self.update_data_instance(probs, doc,min_num_of_sents, max_num_of_sents)
    ...
    def update_data_instance(self, probs, doc, min_num_of_sents=1, max_num_of_sents=3):
        ...
        self.doc = doc
        self.min_num_of_sents = min(min_num_of_sents,self.probs_torch.size()[0])
        self.max_num_of_sents = min(self.probs_torch.size()[0], max_num_of_sents)
    ...
    def generate_index_list_and_loss(self, sample_method="sample"):
        ...
        if sample_method == "lead3":
            ...
            return return_summary_index(self.terminated_way, self.probs_numpy, self.probs_torch,
