...
def reinforce_loss(probs, doc,device, terminated_way, id=0, min_num_of_sents=1,
                   max_num_of_sents=3, max_num_of_bytes=-1,
                   std_rouge=False, rouge_metric="all", compute_score=True):
    # sample sentences
    probs_torch = probs * 0.999 + 0.001
    probs_numpy = probs_torch.data.cpu().numpy()
    probs_numpy = np.reshape(probs_numpy, len(probs_numpy))
    max_num_of_sents = min(probs_torch.size()[0], max_num_of_sents)  # max of sents# in doc and sents# in summary

    rl_baseline_summary_index, _ = return_summary_index(terminated_way, probs_numpy, probs_torch,
                                                        sample_method="greedy", if_test=True, max_num_of_sents=max_num_of_sents,min_num_of_sents=min_num_of_sents)
    ...
    if std_rouge:
        rl_baseline_reward = RougeTest_pyrouge(rl_baseline_ref, rl_baseline_hyp, id=id, rouge_metric=rouge_metric,
                                               compute_score=compute_score, path=os.path.join('../result/greedy',str(device)),
                                               max_num_of_bytes=max_num_of_bytes)
        lead3_reward = RougeTest_pyrouge(lead3_ref, lead3_hyp, id=id, rouge_metric=rouge_metric,
                                         compute_score=compute_score, path=os.path.join('../result/lead',str(device)),
                                         max_num_of_bytes=max_num_of_bytes)
    else:
...
def ext_model_eval(model, vocab, args, eval_data="test"):
    ...
    sample_rewards, eval_rewards, lead3_rewards = [], [], []
    ...
    for phase, dataset in enumerate(data_iter):
        for step, docs in enumerate(dataLoader.BatchDataLoader(dataset, shuffle=False)):
            ...
            if len(doc.content) <3:
                lead3_hyp, lead3_ref = from_summary_index_generate_hyp_ref(doc, range(min(len(doc.content),3)))
                if eval_data == "test":
                    # reward = RougeTest_rouge(lead3_ref, lead3_hyp, rouge_metric='all')
                    reward = RougeTest_pyrouge(lead3_ref, lead3_hyp, id=0, rouge_metric='all',
                            compute_score=compute_score, path=os.path.join('../result/temp',str(args.device)),max_num_of_bytes=args.length_limit)
                else:
                    reward = RougeTest_rouge(lead3_ref, lead3_hyp, rouge_metric=args.rouge_metric)
                lead3_r = reward
            else:
                ...
                if eval_data == "test":
                    # try:
                    reward, lead3_r = reinforce_loss(outputs, doc,args.device, args.terminated_way, id=phase * 1000 + step,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ext_model', type=str, default="SentbySent",
                        help='SentbySent, lstm_summarunner, gru_summarunner, bag_of_words, simpleRNN')
    ...
    parser.add_argument('--pooling_way', type=str, default="lstm_pooling",
                        help="avg_pooling, lstm_pooling, self_att_pooling")
    # local representations CNN parameters
    parser.add_argument('--num_filters', type=int, default=200,
                        help="Number of filters per filter size (default: 200)")
    ...
    parser.add_argument('--terminated_way', type=int, default=0,
                        help='0: binomial; 1: diff')
    parser.add_argument('--novelty', type=bool, default=False,
                        help='when testing, no exploration')
