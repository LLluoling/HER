...
def RougeTest_rouge(ref, hyp, rouge_metric="all", max_num_of_bytes=-1):
    ...
    if isinstance(hyp,types.ListType):
        hyp = ' '.join(hyp).encode('utf-8').lower()
    if isinstance(ref,types.ListType):
        ref = ' '.join(ref).encode('utf-8').lower()
    ...
    elif rouge_metric == 'avg_f_show':
        return np.array([(rouge_score[0]['rouge-1']['f'] + rouge_score[0]['rouge-2']['f'] + rouge_score[0]['rouge-l']['f']) / 3, 
            rouge_score[0]['rouge-1']['f'], rouge_score[0]['rouge-2']['f'], rouge_score[0]['rouge-l']['f'] ])
    else:
...
#         os.mkdir('../result')
#     if not os.path.exists('../result/lead'):
#         os.mkdir('../result/lead')
    ...
    if compute_score:
        if max_num_of_bytes > 0:
            r = Rouge155('%s/pyrouge/tools/ROUGE-1.5.5/' % home_path,
             '-e %s/pyrouge/tools/ROUGE-1.5.5/data -a -c 95 -m -n 2 -b %d' % (home_path,max_num_of_bytes))
        else:
            #r = Rouge155('%s/pyrouge/tools/ROUGE-1.5.5/' % home_path,
            #             '-e %s/pyrouge/tools/ROUGE-1.5.5/data -a -c 95 -m -n 2' % home_path)
            r = Rouge155('%s/pyrouge/tools/ROUGE-1.5.5/' % home_path,
                         '-e %s/pyrouge/tools/ROUGE-1.5.5/data -a -c 95 -m -n 4 -w 1.2' % home_path)
        r.system_dir = path
        r.model_dir = path
        ...
        if rouge_metric[1] == 'f':
            ...
            return np.array([(output_dict["rouge_1_f_score"] + output_dict["rouge_2_f_score"] + output_dict[
                "rouge_l_f_score"]) / 3, output_dict["rouge_1_f_score"], output_dict["rouge_2_f_score"], output_dict["rouge_l_f_score"]])
        elif rouge_metric == 'avg_r':
